import { Options, printHelp } from '../src';
...
describe('cli', () => {
  ...
  it('cli basic', () => {
    let args = ['--input', 'spec/assets/interface1.ts', '--interfaceName', 'Options']
    let p = spawnSync('node', ['bin/extract-interface-docs.js'].concat(args))
  ...
  it('should fail when insufficient args', () => {
    const args = ['--input', 'spec/assets/interface1.ts']
    const p = spawnSync('node', ['bin/extract-interface-docs.js'].concat(args))
